...
from models.systems.choice import ChoiceManager
...
class GameState:
    ...
    @property
    def pending_choice(self) -> ChoiceAction | None:
        return self.choice_mgr.current
    ...
    def apply_damage(self, source: GameCard | None, amount: int, target: GameCard | int, is_combat: bool = False):
        ...
        if is_combat and source and 'Trample' in source.keyword_abilities and isinstance(target, GameCard):
            ...
            if damage_to_player > 0:
                self.score_mgr.decrement_life(target.owner_id, damage_to_player, source)
    ...
    def get_available_actions(self, p_id: int) -> list[Action | CO] | None:
        """Determine what the active player may do right now, in this priority:
                1. Pending choice
                2. State-based actions
                3. Pending choice generated by state-based actions
                4. Game-over handling
                5. Stack interaction
                6. Normal phase actions"""

        # 1. A choice currently requires the player's input
        if self.pending_choice:
            return self.pending_choice.get_actions()

        # 2. State-based processing can itself create a choice
        self.event_mgr.emit(StateBasedEvent())

        # 3. Re-check after state-based processing
        if self.pending_choice:
            return self.pending_choice.get_actions()

        # 4. Game over check & processing
        if self.is_game_over:
            print('The game is over')
            if not self.pending_choice:
                from models.game_over import GameOverChoice
                self.choice_mgr.queue(GameOverChoice(p_id, self))
            return self.pending_choice.get_actions()
