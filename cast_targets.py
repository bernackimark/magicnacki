...
CAST_TARGETS = {
    ...
    'boomerang': lambda gs: CardFilter(gs).in_play().by_type(['Artifact', 'Enchantment', 'Creature', 'Land']).result(),
    ...
    'crumble': lambda gs: CardFilter(gs).in_play().by_type('Artifact').result(),
    'demonic-torment': lambda gs: CardFilter(gs).in_play().creatures().result(),
    'desert-twister': lambda gs: CardFilter(gs).in_play().by_type(['Artifact', 'Enchantment', 'Creature', 'Land']).result(),
    'disenchant': lambda gs: CardFilter(gs).in_play().by_type(['Artifact', 'Enchantment']).result(),
    'divine-offering': lambda gs: CardFilter(gs).in_play().by_type('Artifact').result(),
