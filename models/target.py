...
def create_target_text(targets: int | "GameCard" | "CastToTargetAddToStack" | tuple | list | None):
    ...
    if isinstance(targets, CastToTargetAddToStack):
        return ', targeting ' + targets.card.props.name
    ...
    for t in targets:
        ...
        elif isinstance(t, CastToTargetAddToStack):
            target_text = t.card.props.name
        else:
            raise ValueError(f'{t} must be an int, GameCard, or CastToTargetAddToStack')
