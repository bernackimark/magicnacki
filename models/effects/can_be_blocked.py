...
def artifact_ward_can_be_blocked():
    ...
    class E(Effect):
        ...
        def on_query(self, gs: GameState, event: str, card: GameCard, **kwargs):
            ...
            blocker: GameCard = kwargs.get("blocker")
            if event != "can_block" or not blocker or not card.modifiers.is_enchanted_by('artifact-ward'):
                return None
...
def argothian_pixies_can_be_blocked():
    ...
    class E(Effect):
        ...
        def on_query(self, gs: GameState, event: str, card: GameCard, **kwargs):
            ...
            blocker: GameCard = kwargs.get("blocker")
            if event != "can_block" or card.props.slug != 'argothian-pixies' or not blocker:
                return None
            if 'Artifact' in blocker.props.card_types:
                return False
    return E()
