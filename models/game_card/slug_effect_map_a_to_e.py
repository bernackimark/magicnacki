...
MAP: dict[str, list[EffSpec]] = {
    ...
    'al-abaras-carpet': [Activated('5T', On(DamageProposedEvent, 'EOT').
                                   where(EC().damage_target_in(CF.owner()).damage_source_in(CF.non_fliers())).
    ...
    'ankh-of-mishra': [GenTrig(On(ZoneChangeEvent).where(EC().to_board().card_is_land()).
                       then(DealDamage(2, ET.event_card_owner())))],
    ...
    'argothian-pixies': [Static(UnblockableCondition(CF.self(), CF.artifact_creatures())),
                         GenTrig(On(DamageProposedEvent).
                                 where(EC().damage_target_in(CF.self()).damage_source_in(CF.artifact_creatures())).
    ...
    'citanul-druid': [GenTrig(On(CastResolvedEvent).where(EC().caster_is_opp().card_is_artifact()).then(AddCounter(PLUS_ONE)))],
    ...
    'demonic-torment': [Spell(HostCantAttack(), CF.creatures()),
                        GenTrig(On(DamageProposedEvent, expires='EOT').
                                where(EC().is_combat_damage().damage_source_in(CF.host())).modify(PreventDamage()))],
    ...
    'dingus-egg': [GenTrig(On(ZoneChangeEvent).where(EC().from_board().to_gy().card_is_land()).
                           then(DealDamage(2, ET.event_card_owner())))],
    ...
    'enchanted-being': [GenTrig(On(DamageProposedEvent).
                                where(EC().damage_target_in(CF.self()).damage_source_in(CF.enchanted_creatures()).is_combat_damage()).modify(PreventDamage()))],
