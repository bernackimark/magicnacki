...
MAP: dict[str, list[EffSpec]] = {
    ...
    'clockwork-avian': [Static(ClockworkCombatEnd()),
                        Activated('XT', AddCounter(PLUS_ONE_ZERO), T_FUNCS['self'], allowed_phases=[Phase.UPKEEP],
                                  allowed_p_turn_func=T_FUNCS['card_owner'], max_x_func=clockwork_avian_x),
                        Spell(AddCounter(PLUS_ONE_ZERO, 4))],
    'clockwork-beast': [Static(ClockworkCombatEnd()),
                        Activated('XT', AddCounter(PLUS_ONE_ZERO), T_FUNCS['self'], allowed_phases=[Phase.UPKEEP],
                                  allowed_p_turn_func=T_FUNCS['card_owner'], max_x_func=clockwork_beast_x),
    ...
    'disrupting-scepter': [Activated('3T', Discard(), T_FUNCS['all_players'],
                                     allowed_p_turn_func=T_FUNCS['card_owner'])],
