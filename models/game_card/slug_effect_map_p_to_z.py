...
MAP: dict[str, list[EffSpec]] = {
    ...
    'psychic-allergy': [Triggered(PsychicAllergySac(), T_FUNCS['self']),
                        Spell(PsychicAllergyDamage()), Triggered(DeclareAColor())],
    'psychic-purge': [Spell(DealDamage(1), T_FUNCS['all_creatures_and_players']),
                      Triggered(PsychicPurgeDiscard())],
    'psychic-venom': [Spell(None, T_FUNCS['lands']), Triggered(PsychicVenom())],
    'purelace': [Spell(SetColor('W'), T_FUNCS['cards'])],
    'pyrotechnics': [Spell(DealOneDamageToTargetList(),
                           TargetSpec(T_FUNCS['all_creatures_and_players'], 1, 4, allow_duplicate_targets=True))],
    ...
    'rasputin-dreamweaver': [Activated('', AddMana('C'), extra_costs=[RemoveCounterCost(DREAM)]),
                             ...
                             Spell(AddCounter(DREAM, 7))],  # more to code
